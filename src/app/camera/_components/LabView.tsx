...
export default function LabView({
  ...
  const filters = [
    { id: "none", label: "Original", class: "" },
    {
      id: "grayscale",
      label: "Deep Analog",
      class: "grayscale contrast-125 brightness-90",
    },
    {
      id: "sepia",
      label: "Creamy Film",
      class: "sepia-[0.5] contrast-110 saturate-80",
    },
    {
      id: "classic",
      label: "Classic B&W",
      class: "grayscale sepia-[0.2] contrast-150",
    },
    {
      id: "vivid",
      label: "Vivid Retro",
      class: "contrast-110 brightness-110 saturate-125",
    },
  ];

  // Inisialisasi audio sejak awal secara aman
  useEffect(() => {
    if (!audioRef.current) {
  ...
  const triggerAutomaticPrintSound = useCallback(() => {
    if (!audioRef.current || hasPlayedRef.current || !audioUnlocked) return;
    ...
    audioRef.current
      ...
      .then(() => {
        ...
        console.log("🖨️ Printer sound playing");
      })
      .catch((err) => console.log("Audio error:", err));
  }, [audioUnlocked]);

  useEffect(() => {
    if (!audioUnlocked) return; // ⛔ tunggu audio unlock dulu
  ...
  }, [triggerAutomaticPrintSound, audioUnlocked]);
      ...
      await new Promise((resolve) => {
        const processImage = () => {
          ...
          if (el.type === "photo") {
            ...
            // 💡 FIX FILTER MUTLAK IPHONE: Bikin satu canvas bayangan berukuran pas sesuai box elemen
            ...
            if (oCtx) {
              // Setel filter langsung ke objek induk tunggal WebKit Safari menggunakan String Gambar Murni
              if (filter === "grayscale") {
                oCtx.filter = "grayscale(100%) contrast(120%) brightness(95%)";
              } else if (filter === "sepia") {
                oCtx.filter = "sepia(60%) contrast(110%) brightness(100%)";
              } else if (filter === "classic") {
                oCtx.filter = "grayscale(100%) sepia(25%) contrast(135%)";
              } else if (filter === "vivid") {
                oCtx.filter = "contrast(115%) brightness(105%) saturate(130%)";
              } else {
                oCtx.filter = "none";
              }
              ...
              // 💡 KUNCI UTAMA SAFARI: Menggambar wajib murni dari objek Image (img), bukan dari objek Canvas penengah!
              oCtx.drawImage(img, sX, sY, sW, sH, 0, 0, el.w, el.h);

              // Pindahkan hasil akhir canvas berfilter langsung ke canvas utama
        ...
        if (img.complete) {
          // Kasih jeda waktu aman 30ms biar sinkronisasi render thread WebKit gak tabrakan
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      renderFinalCollage();
    }, 60); // delay kecil biar state settle

    return () => clearTimeout(timeout);
  }, [filter, images, template]);

  const handleFilterClick = (filterId: string) => {
    setFilter(filterId); // Murni cuma ganti state filter, gak bakal ganggu sound printer lagi!
  };
  ...
  const handleFinalizeAndUpload = async () => {
    ...
    try {
      ...
      if (filter === "grayscale") gifFilterString = "grayscale(100%)";
        ...
        gifFilterString = "grayscale(100%) sepia(20%)";
  ...
  return (
    <motion.div
      ...
      onTouchStart={unlockAudioContext} // Sentuhan pertama user otomatis melepas kunci audio iPhone!
      onClick={unlockAudioContext}
    >
      <div
        className={`flex flex-row w-full h-full max-w-[1400px] items-center justify-center ${isMobileView ? "gap-0" : "gap-10"}`}
      >
        {/* PRINTER AREA */}
        <div className="flex-[1.5] h-full flex flex-col items-center justify-center relative">
          <div
            className={`relative flex flex-col items-center transition-all duration-700 ${isMobileView ? "scale-[0.89] -translate-x-6" : "scale-90 md:scale-100"}`}
          >
            <div
              className={`${isMobileView ? "w-[300px] h-18" : "w-[400px] h-28"} bg-[#222224] rounded-t-[4rem] z-40 shadow-2xl border-x-[15px] border-[#161618] relative flex flex-col items-center`}
            >
              <div className="mt-4 px-3 py-1 border border-white/5 rounded-full">
                <p className="text-[4px] md:text-[6px] font-black text-white/30 uppercase tracking-[0.5em] italic leading-none">
                  Caméree Printer
                </p>
              </div>
            ...
            <div
              className={`${isMobileView ? "w-[260px] h-20" : "w-[340px] h-30"} absolute top-3 bg-black/90 rounded-full shadow-2xl -mt-8 z-10`}
            />
            <div
              className={`${isMobileView ? "w-[260px] h-20" : "w-[340px] h-30"} absolute top-2/12 md:top-2/11 bg-black/90 shadow-2xl -mt-8 z-10`}
            />
            <div
              className={`${isMobileView ? "w-[263px] h-6" : "w-[343px] h-10"} bg-[#100f0f] rounded-full shadow-2xl -mt-4 md:-mt-6 z-50`}
            />

            <div
              className={`relative z-10 overflow-hidden ${isMobileView ? "h-[62vh] w-[360px]" : "h-[62vh] w-[460px]"} flex justify-center -mt-1 pointer-events-none`}
            >
              <motion.div
                initial={{ y: -800 }}
                animate={{ y: 0 }}
              ...
              >
                <canvas
                  ref={canvasRef}
                  className="max-h-[62vh] w-auto h-auto block"
                />
              </motion.div>
            </div>
          </div>
        ...
        {/* SIDEBAR TOOLS */}
        <div
          className={`flex flex-col bg-white/40 backdrop-blur-3xl rounded-[2.5rem] md:rounded-[3.5rem] border border-white/50 z-50 shadow-2xl shrink-0 transition-all h-[90vh] ${isMobileView ? "w-xs p-5 mr-8" : "w-72 md:w-xl p-8"}`}
        >
          <div className="flex items-center gap-2 opacity-40 mb-4">
            <Loader2 size={14} className="text-[#153378]" />
            <h3 className="text-[8px] md:text-[10px] font-black text-[#153378] uppercase tracking-[0.3em]">
              Filter
            </h3>
          </div>
          ...
          >
            {filters.map((f) => (
              ...
              >
                <div
                  className={`w-10 h-10 md:w-14 md:h-14 rounded-xl overflow-hidden shrink-0 shadow-inner ${f.class}`}
                >
                ...
                <div className="flex flex-col items-start overflow-hidden leading-tight">
                  <span className="text-[9px] md:text-[11px] font-black uppercase truncate w-full tracking-tighter">
                    {f.label}
                  </span>
                  <span className="text-[6px] md:text-[7px] font-bold uppercase opacity-50 tracking-widest">
                    Select
                  </span>
                </div>
              </button>
          ...
          <div className="mt-3 md:mt-6 space-y-3 shrink-0 border-t border-[#153378]/10 pt-3 md:pt-6">
            {!isDone ? (
              ...
              >
                ...
                <span className="uppercase italic text-[8px] md:text-[10px] tracking-widest">
                  {isUploading ? "..." : "Softfile"}
                </span>
              </button>
            ) : (
